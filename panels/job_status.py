...
class JobStatusPanel(ScreenPanel):
    ...
    def create_status_grid(self, widget=None):
        ...
        self.buttons['layers']  = self._gtk.Button("layer", layer_label, None, self.bts, Gtk.PositionType.LEFT, 1) # Changes
        self.buttons['layers'] .connect("clicked", self.create_move_grid) # Changes
        self.buttons['layers'] .set_halign(Gtk.Align.START) # Changes

        szfe = Gtk.Grid()
        szfe.attach(self.buttons['speed'], 0, 0, 1, 1)
        szfe.attach(self.buttons['z'], 1, 1, 1, 1) # Changes
        szfe.attach(self.buttons['elapsed'], 0, 2, 1, 1) # Changes
        szfe.attach(self.buttons['left'], 1, 2, 1, 1) # Changes
        if self._screen.printer.get_tools():
            szfe.attach(self.buttons['extrusion'], 0, 1, 1, 1)
        if self._screen.printer.get_fans():
            szfe.attach(self.buttons['fan'], 1, 0, 1, 1) # Changes

        info = Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
        info.get_style_context().add_class("printing-info")
        info.add(self.labels['temp_grid'])
        info.add(szfe)
        info.add(self.buttons['layers']) # Changes
