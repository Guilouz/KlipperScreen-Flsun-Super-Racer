...
class Panel(ScreenPanel):
    ...
    def create_status_grid(self, widget=None):
        ...
        #szfe.attach(self.buttons['z'], 2, 0, 2, 1) # Changes
        ...
        if self._printer.get_fans():
            #szfe.attach(self.buttons['fan'], 2, 1, 2, 1) # Changes
        ...
        #info.attach(self.buttons['elapsed'], 0, 3, 1, 1) # Changes
        #info.attach(self.buttons['left'], 0, 4, 1, 1) # Changes
    ...
    def process_update(self, action, data):
        ...
        if "gcode_move" in data:
            with suppress(KeyError):
                ...
                #self.buttons['z'].set_label(f"Z: {self.pos_z:6.2f}{f'/{self.oheight}' if self.oheight > 0 else ''}") # Changes
            ...
            if 'info' in data["print_stats"]:
                with suppress(KeyError):
                    if data["print_stats"]['info']['total_layer'] is not None:
                        ...
                        self.buttons['layers'].set_label(f"{self.labels['layer_lbl'].get_text()} {data['print_stats']['info']['total_layer']}") # Changes
                with suppress(KeyError):
                    if data["print_stats"]['info']['current_layer'] is not None:
                        ...
                        self.buttons['layers'].set_label(f"{self.labels['layer_lbl'].get_text()} {data['print_stats']['info']['current_layer']} / {self.labels['total_layers'].get_text()}") # Changes
                ...
                self.buttons['layers'].set_label(f"{self.labels['layer_lbl'].get_text()} {1 + round((self.pos_z - self.f_layer_h) / self.layer_h)} / {self.labels['total_layers'].get_text()}") # Changes
    ...
    def show_buttons_for_state(self):
        ...
        if self.state == "printing":
            ...
            if self.state == "cancelling": # Changes
                self.buttons['button_grid'].attach(Gtk.Label(""), 0, 0, 1, 1) # Changes
                self.buttons['button_grid'].attach(Gtk.Label(""), 1, 0, 1, 1) # Changes
                self.buttons['button_grid'].attach(Gtk.Label(""), 2, 0, 1, 1) # Changes
                self.buttons['button_grid'].attach(self.buttons['wait'], 3, 0, 1, 1) # Changes
            if self.state != "cancelling": # Changes
                self.buttons['button_grid'].attach(Gtk.Label(""), 0, 0, 1, 1) # Changes
                self.buttons['button_grid'].attach(Gtk.Label(""), 1, 0, 1, 1) # Changes
                self.buttons['button_grid'].attach(self.buttons['restart'], 2, 0, 1, 1) # Changes
                self.buttons['button_grid'].attach(self.buttons['menu'], 3, 0, 1, 1) # Changes
                self.can_close = True # Changes
