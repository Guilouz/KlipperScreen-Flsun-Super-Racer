...
class Panel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        ...
        if self._config.get_main_config().getboolean('touch_sound', True): # Changes
            self.labels['menu'].connect("pressed", self._screen._button_pressed_feedback) # Changes
        ...
        if self._config.get_main_config().getboolean('touch_sound', True): # Changes
            self.labels['restart'].connect("pressed", self._screen._button_pressed_feedback) # Changes
        self.labels['firmware_restart'] = self._gtk.Button("refresh", _("Firmware Restart"), "color2")
        self.labels['firmware_restart'].connect("clicked", self.firmware_restart)
        if self._config.get_main_config().getboolean('touch_sound', True): # Changes
            self.labels['firmware_restart'].connect("pressed", self._screen._button_pressed_feedback) # Changes
        ...
        if self._config.get_main_config().getboolean('touch_sound', True): # Changes
            self.labels['restart_system'].connect("pressed", self._screen._button_pressed_feedback) # Changes
        ...
        if self._config.get_main_config().getboolean('touch_sound', True): # Changes
            self.labels['shutdown'].connect("pressed", self._screen._button_pressed_feedback) # Changes
        self.labels['retry'] = self._gtk.Button("load", _('Retry'), "color3")
        self.labels['retry'].connect("clicked", self.retry)
        if self._config.get_main_config().getboolean('touch_sound', True): # Changes
            self.labels['retry'].connect("pressed", self._screen._button_pressed_feedback) # Changes
    ...
    def add_power_button(self, powerdevs):
        ...
        if self._config.get_main_config().getboolean('touch_sound', True): # Changes
            self.labels['power'].connect("pressed", self._screen._button_pressed_feedback) # Changes
    ...
    def reboot_poweroff(self, widget, method):
        ...
        if method == "reboot":
            ...
            title = _("Shutdown")
            buttons = [
                #{"name": _("Host"), "response": Gtk.ResponseType.OK, "style": 'dialog-info'}, # Changes
                {"name": _("Shutdown"), "response": Gtk.ResponseType.APPLY, "style": 'dialog-warning'}, # Changes
                {"name": _("Cancel"), "response": Gtk.ResponseType.CANCEL, "style": 'dialog-error'}
            ]
        #if self._screen._ws.connected: # Changes
            #buttons.insert(1, {"name": _("Printer"), "response": Gtk.ResponseType.APPLY, "style": 'dialog-warning'}) # Changes
